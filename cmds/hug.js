...
module.exports = {
    run: function (djs, m, args) {
      ...
      if(m.mentions.members.size > 1)
        return m.channel.send("You're such a great Care Bear. Unfortunately, I'm not. (Mention only one user to hug)");
      let u = m.guild.member(m.mentions.members.first());
      nfetch("https://nekos.life/api/hug", { method: 'GET' })
      .then(res => res.json()).then(json => {
        ...
        console.error(`Fatal error when fetching a hug pic. Here's the error: ${err}`);
